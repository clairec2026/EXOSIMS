...
class TestMLR(unittest.TestCase):
    ...
    def test_format(self):
        ...
        ** Test only runs with FGK stars.
        ...
        TList.massLuminosityRelationship = "Torres2009"
        print("Checking " + str(TList.massLuminosityRelationship) + " values")
        ind = 0
        for mass in TList.MsTrue:
            val = TList.MsTrue[ind].value.item()
    ...
    def test_input(self):
        ...
        self.spec["modules"]["massLuminosityRelationship"] = "Torres2009"
        TList = TargetList(
            **copy.deepcopy(self.spec)
        )
        self.assertEqual(TList.massLuminosityRelationship, "Torres2009")
    ...
    def test_calc(self):
        ...
        # Torres2009
        print("Checking Torres2009 calculations")
        TList.massLuminosityRelationship = "Torres2009"
        TargetList.stellar_mass(TList)
        MsEst = (10 ** (1.5689 + 1.3787 * (np.log(TList.Teff.value) - 4.1) +
                        0.4243 * ((np.log(TList.Teff.value)-4.1) ** 2) + 1.139
                        * ((np.log(TList.Teff.value)-4.1) ** 3)))
        ind = 0
        for mass in MsEst:
            self.assertEqual(TList.MsEst.value[ind], MsEst[ind])
        ind += 1
